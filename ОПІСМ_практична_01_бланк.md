...
|---|---|---|---|
| 1 |Прикладний вміст сторінки (HTML) |html,head title 301 Moved Permanently /title /head, /html|Фінальний вміст сторінки, який відображається браузером користувача |
| 2 |Відповідь від сервера|< HTTP/1.1 301 Moved Permanently, < Location: https://www.ietf.org/, < Server: cloudflare|Службові заголовки сервера про успішність запиту та тип даних |
| 3 |Запит від клієнта | > GET / HTTP/1.1, > Host: ietf.org, > User-Agent: curl/8.21.0|Команда прикладного рівня, яку curl надсилає для запиту ресурсу |
| 4 |Встановлення захищеного каналу (TLS/SSL) |* ALPN: curl offers http/1.1, * schannel: renegotiating SSL/TLS connection |Етап узгодження шифрування та перевірки сертифіката перед відправкою HTTP-запиту |
